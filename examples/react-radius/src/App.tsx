...
import { radiusTestnet } from '@stablecoin.xyz/core';
...
function WalletStatus({ onDisconnect }: { onDisconnect: () => void }) {
  ...
  const formatTestSbcBalance = (balance: string | null): string => {
    ...
    try {
      return (Number(balance) / Math.pow(10, TEST_SBC_DECIMALS)).toFixed(2);
    } catch {
      return '0.00';
    }
  };
          ...
          <div className="mt-2 pt-2 border-t border-green-200">
            ...
            {isLoadingBalances ? (
              ...
              <div className="flex gap-4">
                ...
                <span className="text-xs text-green-600">
                  <strong>TestSBC:</strong> {formatTestSbcBalance(balances.testSbc)}
                </span>
              </div>
            )}
          </div>
...
function SmartAccountInfo() {
  ...
  // Fetch TestSBC balance for smart account
  useEffect(() => {
    ...
    const fetchTestSbcBalance = async () => {
      ...
      try {
        ...
        console.error('Failed to fetch TestSBC balance for smart account:', error);
        setTestSbcBalance('0');
      } finally {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      // Refresh TestSBC balance as well
      if (account?.address) {
        setIsLoadingBalance(true);
        try {
          const balance = await publicClient.readContract({
          ...
          console.error('Failed to refresh TestSBC balance:', error);
        } finally {
          setIsLoadingBalance(false);
        }
      }
      ...
      <div className="space-y-2 text-sm">
        ...
        <div className="pt-2 border-t border-purple-200">
          ...
          <div className="space-y-1">
            ...
            <div className="flex justify-between">
              <span className="text-purple-700">TestSBC:</span>
              <span className="text-purple-600 font-mono text-xs">
                {isLoadingBalance ? 'Loading...' : `${formatTestSbcBalance(testSbcBalance)} TestSBC`}
              </span>
            </div>
          </div>
        </div>
...
function TransactionForm() {
  ...
  const handleSendTransaction = async () => {
    ...
    try {
      ...
      const value = parseUnits(amount, TEST_SBC_DECIMALS);
      ...
      const signature = await getPermitSignature({
        ...
        value,
        tokenAddress: TEST_SBC_TOKEN_ADDRESS,
        chainId: chain.id,
        deadline,
      });
      ...
      await sendUserOperation({
        calls: [
          { to: TEST_SBC_TOKEN_ADDRESS as `0x${string}`, data: permitCallData },
          { to: TEST_SBC_TOKEN_ADDRESS as `0x${string}`, data: transferFromCallData },
        ],
      });
    } catch (err) {
  ...
  return (
    <div className="p-4 bg-white border border-gray-200 rounded-lg shadow-sm">
      <h3 className="font-semibold text-gray-800 mb-4">💸 Send TestSBC Tokens</h3>
      <div className="space-y-4">
        ...
        <div className="p-3 bg-gray-50 rounded">
          <div className="flex justify-between text-sm">
            <span>Amount:</span>
            <span className="font-medium">{amount} TestSBC</span>
          </div>
        ...
        >
          {isLoading ? 'Waiting for signature...' : `Send ${amount} TestSBC`}
        </button>
        {isSuccess && data && (
        ...
        <div className="max-w-2xl mx-auto px-4">
          <div className="text-center mb-8">
            ...
            <p className="text-xs text-gray-500 mt-1">
              TestSBC Token: {TEST_SBC_TOKEN_ADDRESS}
            </p>
          </div>
          <WalletConnectFlow />
...
async function getPermitSignature({
  ...
  try {
    ...
    const domain = {
      ...
      verifyingContract: TEST_SBC_TOKEN_ADDRESS as `0x${string}`,
    };
